...
create_network <- function(num_tfs,
                           ...
                           num_hks,
                           bio_net=NULL,
                           bio_net_name=sample(grn_names, size=1),
                           verbose=FALSE) {
...
get_network <- function(bio_net_name, net_dir='../data/fantom5') {
    ...
    bio_net_fp <- file.path(net_dir, paste0(bio_net_name, '.Rds'))
    adj_mat <- readRDS(bio_net_fp)
