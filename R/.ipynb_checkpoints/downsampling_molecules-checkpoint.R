#' @export
downsampling_molecules <- function(model, ncells=NA, ncells_per_grna=100, map_reference_ls=TRUE,
                                   map_reference_cpm=TRUE, return_realcount=FALSE,
                                   realcount_dataset=NA) {
    
    check_experiment_parameters(model, ncells, ncells_per_grna)
    realcount <- get_realcounts(realcount_dataset, model$simulation_system$cache_dir)
    ...
    spliced_counts <- sim_counts[, model$feature_info$mol_mrna, drop = FALSE]
    unspliced_counts <- sim_counts[, model$feature_info$mol_premrna, drop = FALSE]
    
    # rename columns by genes and rownames by cell
    dimnames(spliced_counts) <- list(cell_info$cell_id, model$feature_info$feature_id)
    dimnames(unspliced_counts) <- list(cell_info$cell_id, model$feature_info$feature_id)
    
    # add together unspliced and spliced mrna
    counts <- unspliced_counts + spliced_counts
    model$experiment <- lst(cell_info, counts, unspliced_counts, spliced_counts)
    
    if (return_realcount) {
        model$downsampling <- lst(realcount_dataset, realcount)
    }
...
#' @export
get_realcounts <- function(dataset, cache_dir) {
    
    if (is.na(dataset)) {
        dataset <- sample(realcounts$filename, size=1)
    }
    
    url <- realcounts %>% filter(filename == dataset) %>% pull(links.download)
    realcount <- download_cacheable_file(url, cache_dir, TRUE)
...
sample_molecules <- function(tsim_counts_cpm, lib_size_new) {
    ...
    new_vals <- lapply(seq_along(lib_size_new), function(cell_i) {
                        val <- integer(0)
                        ...
                        if (pi != pj) {
                            ...
                            val <- rmultinom(1, lib_size, gene_vals)
                        }
