...
get_grna_multiplier <- function(grna, crispr_type) { 
    ...
    if (crispr_type == 'Knockout' & ngenes > 0) {
        ...
        grna_multipliers <- list(c(0, 1))
        grna_multipliers[[grna$gene]] <- c(0, 1)

        grna_multipliers <- grna_multipliers %>% as.data.frame
        model_names <- c(paste0(grna_name, '_', grna$gene, '-PRT'),
                         paste0(grna_name, '_', grna$gene, '-NT'))
    } else {
...
calculate_sample_percentages <- function(model){
    ...
    for (sim_name in rownames(multiplier)) {
        ...
        probs_mat <- NULL
        grna_name <- str_split(sim_name, "_")[[1]][1]
        grna_meta <- model$grnas_meta %>% filter(grna == grna_name)
        ko_combos <- multiplier[sim_name, grna_meta$gene]

        for (col_name in colnames(ko_combos)) { 
            gene_on_target <- grna_meta %>%
                                filter(gene == col_name) %>%
