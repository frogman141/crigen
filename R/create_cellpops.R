...
#' @export
create_cellpops <- function(crigen_obj) {
    params <- crigen_obj$cellpop_params
    
    ncellpops <- 1:params$ncellpops
    cellpop_names <- paste0('CellPop', ncellpops, '_')
    
    cellpop_meta <- construct_populations(ncellpops, cellpop_names, params)
    
    branch_meta <- create_branches(ncellpops, cellpop_names, cellpop_meta$module_info)
    start_branch <- bblego_start('B', type = "simple", num_modules = 2)
    start_branch$expression_patterns$time = 25
    
    crigen_obj$simulator_params$backbone <- bblego(start_branch, branch_meta, cellpop_meta)
    return (crigen_obj)
}

construct_populations <- function(n_cellpops, cellpop_names, params) {
    ...
    for (i in n_cellpops) {
        ...
        ntfs <- params$ntfs_per_cellpop[i]

        # generate graph with n transcription factors
        generated_net <- erdos.renyi.game(ntfs, params$edge_prob, directed=TRUE, loops=TRUE)
        ...
        meta <- list(module_info=module_info,
